...
\tmcolor{blue}{Note that $\nabla \equiv \partial / \partial \mathbf{X}$. Using
$\mathbf{x}=\mathbf{X}+\mathbf{\rho}$, we obtain
\begin{eqnarray}
  \frac{\partial \delta \Phi_g (\mathbf{X}, \alpha, v_{\perp})}{\partial
  \mathbf{X}} & = & \frac{\partial \delta \Phi_p (\mathbf{X}+\mathbf{\rho}
  (\mathbf{X}, \alpha, v_{\perp}))}{\partial \mathbf{X}} \nonumber\\
  & = & \frac{\partial \delta \Phi_p (\mathbf{x})}{\partial \mathbf{X}} =
  ...
  \mathbf{x}}, 
\end{eqnarray}
which is accurate up to $O (\lambda)$ in gyrokinetic ordering. Note that field
values on a regular $\mathbf{X}$ grid are not available in our simulation
because $\mathbf{X}$ is sampled by random markers rather than by a regular
grid. This fact makes it diffcult to numerical compute $\partial / \partial
\mathbf{X}$. Then the relation $\partial \delta \Phi_g / \partial \mathbf{X}
\approx \partial \delta \Phi_p / \partial \mathbf{x}$, becomes useful because
we can use $\delta \Phi$ defined on $\mathbf{x}$ grid (which is available in
simulations) to compute \ $\partial \delta \Phi_p / \partial \mathbf{x}$ at
$\mathbf{x}$ grid-points (using finite-difference), and then interpolate it to
particle position. \ Similarly, we have $\partial \delta A_{\parallel g} /
\partial \mathbf{X} \approx \partial \delta A_{\parallel p} / \partial
\mathbf{x}$.
...
\end{equation}
For notation ease, the subscripts $g$ and $p$ will be dropped in the
following.}
...
\begin{figure}[h]
  ...
  two buffer regions (regions in blue color). In the code $n$ is denoted by \tmtexttt{nflux2} and $\tmop{nt}$ is denoted
