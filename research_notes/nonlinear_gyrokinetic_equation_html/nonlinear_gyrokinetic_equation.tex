...
transporting plasma in tokamaks.
...
O (\lambda^1)$, $\partial F_0 / \partial \varepsilon \sim - F_0 m / T$, and
$\rho \nabla F_0 \sim O (\lambda^1) F_0$, we find that \tmcolor{red}{the red
term} of the above equation (after divided by $\Omega F_0$) is of order $O
(\lambda^3)$ and hence can be dropped. Move the second line to the right-hand
side, giving
\begin{eqnarray}
  &  & \left[ \frac{\partial}{\partial t} + (v_{\parallel}
  \mathbf{e}_{\parallel} +\mathbf{V}_D + \delta \mathbf{V}_D) \cdot \nabla_X
...
\tmtexttt{GEM} code manual (the first line of Eq. 28). (It is said that
...
\subsubsection{Mixed variable pullback method{\cite{MISHCHENKO2019194}}}
...
\begin{equation}
  \label{23-11-16-1} \frac{\partial \delta A_{\parallel}^{(s)}}{\partial t} =
  0,
\end{equation}
or
\begin{equation}
  ...
  -\mathbf{b} \cdot \nabla \delta \Phi .
\end{equation}
Following the same procedure as the above, we obtain an evoluation equation
for $\delta f^{(h)}$:
\begin{eqnarray}
  &  & \left[ \frac{\partial}{\partial t} + (v_{\parallel}
  \mathbf{e}_{\parallel} +\mathbf{V}_D + \delta \mathbf{V}_D) \cdot \nabla_X
  ...
  \frac{\partial F_0}{\partial \varepsilon},  \label{23-12-4-2}
\end{eqnarray}
if we adopt Eq. (\ref{23-11-16-1}) for $\delta A_{\| \nobracket}^{(s)}$; or
\begin{eqnarray}
  &  & \left[ \frac{\partial}{\partial t} + (v_{\parallel}
  \mathbf{e}_{\parallel} +\mathbf{V}_D + \delta \mathbf{V}_D) \cdot \nabla_X
  \right] \delta f^{(h)} \nonumber\\
  ...
  \frac{\partial F_0}{\partial \varepsilon}, 
\end{eqnarray}
if we adopt Eq. (\ref{23-11-16-2}) for $\delta A_{\|
\nobracket}^{(s)}$.simulations, disappear on the right-hand side of the
evolution equation of $\delta f^{(h)}$.
...
distrbution funciton $\delta f^{(h)}_s$, where $s$ is species index. Here
$\delta A_{\parallel}^{(s)}$ has been moved to the right-hand side since its
value is already known when we solve the Ampere's law for $\delta
A_{\parallel}^{(h)}$.
...
equation (user's choice) and the remainder is solved from the Ampere's law. We
hope that the former is the dominant part of $\delta A_{\parallel}$ so that
the remainder solved from the Ampere's law is small and hence the cancellation
problem of the Ampere's law become less significant. While a wise choice of
the evolution equation for $\delta A_{\parallel}^{(s)}$ may help ensure that
$\delta A_{\parallel}^{(s)}$ remain dominant over the entire simulation
duration, we have a simple and efficient way to make $\delta
A_{\parallel}^{(s)}$ remain dominant: collect both $\delta
...
remain unchanged), we need set $\delta A_{\parallel}^{(h)}$ to zero:
\begin{equation}
...
\end{equation}
where $q_u, n_u, T_u, m_u, v_u$ are chosen units, $v_{n s}$ is a velocity
unit, whose value is different among different species. Then the Ampere's law
(\ref{23-12-4-1}) is written as
\begin{equation}
...
\end{equation}
where use has been made of $\mu_0 = 1 / (c^2 \varepsilon_0)$ and $\omega_{p
u}^2 = n_u q_u^2 / (m_u \varepsilon_0)$.
...
\end{eqnarray*}
In terms of units used in TEK, Eq. (\ref{23-11-16-2}) is written as


\[ \frac{\partial \delta \overline{A}_{\parallel}^{(s)}}{\partial
   \overline{t}} \frac{T_u}{q_u v_{n i}}  \frac{1}{t_{n i}} = - \frac{T_u}{q_u
   L_n} \mathbf{b} \cdot \overline{\nabla} \delta \Phi, \]
where $v_{n i}$ and $t_{n i}$ are units of velocity and time for the main ion
species. Then the above equation is written as
\begin{equation}
...
\begin{figure}[h]
  ...
  \tmtexttt{nth} is denoted by \tmtexttt{ntheta} in GEM.
  ...
  to be consistent.\quad TEK poloidal grid-points for perturbation are indexed
  as \tmtexttt{0:mpol2} with the index \tmtexttt{0} corresponding to $\theta =
  - \pi$ and the index \tmtexttt{mpol2} corresponding to $\theta = + \pi$.
  Field equation is solved at \tmtexttt{$0 : \tmop{mpol} 2 - 1$}. (The field
  at \tmtexttt{mpol2}, i.e., $\theta = + \pi$, is obtained by inerpolating the
  field at $\theta = - \pi$.) \tmtexttt{mpol2} is determined by
  \tmtexttt{mpol2=numproc/ntube}, i.e., \tmtexttt{mpol2} is the total number
  of cells along the $\theta$ direction use for MPI parallelation.
  \tmtexttt{mpol} and \tmtexttt{mpol2} must be chosen in a way that makes
  \tmtexttt{(mpol-1)/mpol2} be an integer, which can be larger than 1, i.e.,
  one simulation cell can span several equilibrium cells.}
\end{figure}
...
\begin{figure}[h]
  ...
  grid, and perturbations are defined on the latter grid.
  Here $\tmop{nt} = n + 2 m$, $m$ is the number of grid points in one of the
  two buffer regions (regions in blue color).
  
  In the code $n$ is denoted by \tmtexttt{nflux2} and $\tmop{nt}$ is denoted
  by \tmtexttt{nflux}, $m$ is denoted by \tmtexttt{points\_in\_buffer}. In
  \tmtexttt{GEM} the radial array does no include the buffer regions and the
  index starts at \tmtexttt{0} and ends at \tmtexttt{imx}. Hence
  \tmtexttt{n=imx+1}.}
\end{figure}

\begin{figure}[h]
  \includegraphics{/home/yj/theory/figures/my_grids_and_gem_grids/toroidal_grids/a-1.eps}
  \caption{Toroidal grid used in \tmtexttt{TEK} and GEM. TEK array index
  starts at 1 whereas GEM array index starts from $0$. In TEK, the toroidal
  array is \tmtexttt{tor\_1d\_array(1:mtor+1)}. source terms, $\delta n$ and
  $\delta j_{\parallel}$, are defined at \tmtexttt{1:mtor}; EM fields are
  defined at \tmtexttt{1:mtor+1}.
  \tmtexttt{GEM} array ends at \tmtexttt{jm}. It follows that
  \tmtexttt{mtor=jm}.}
\end{figure}
