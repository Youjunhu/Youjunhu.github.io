...
perturbed particle distribution function in the phase space can be established
if we choose suitable coordinates (independent variables) for the phase space
and split the distribution function in a proper way. The symmetry is along the
...
\subsection{Vlasov equation and Gyro-angle symmetry of distribution
function}\label{19-4-23-p1}

The Vlasov equation in terms of particle coordinates $(\mathbf{x},
\mathbf{v})$ is written
\begin{equation}
  \label{22-11-12-1} \frac{\partial f_p}{\partial t} +\mathbf{v} \cdot
  \frac{\partial f_p}{\partial \mathbf{x}} + \frac{q}{m}
  (\mathbf{E}+\mathbf{v} \times \mathbf{B}) \cdot \frac{\partial f_p}{\partial
  \mathbf{v}} = 0,
\end{equation}
...
Let us consider a simple case where the electromagnetic field is a
time-independent and spatial uniform magnetic field, $\mathbf{B}= B_0
\hat{\mathbf{z}}$. And examine the Vlasov equation in this simple case and
look if there is any coordinate system that can make the distribution function
be uniform in one of its coordinates, i.e., being symmetric in this
coordinate.
...
z, v_{\perp}, \alpha, v_{\parallel})$. The distribution function $f_p$ depends
...
in velocity space, $\partial f_p /\mathbf{v}$, is written as


\begin{equation}
  \frac{\partial f_p}{\partial \mathbf{v}} = \frac{\partial f_p}{\partial
  v_{\perp}} \mathbf{e}_1 + \frac{\partial f_p}{\partial v_{\parallel}}
  \mathbf{e}_z + \frac{1}{v_{\perp}} \frac{\partial f_p}{\partial \alpha}
  \mathbf{e}_{\alpha},
\end{equation}
...
\mathbf{B}) \cdot \frac{\partial f_p}{\partial \mathbf{v}}$ is written as
\begin{eqnarray}
  \frac{q}{m} (\mathbf{v} \times \mathbf{B}) \cdot \frac{\partial
  f_p}{\partial \mathbf{v}} & = & \frac{q}{m} (v_{\perp} \mathbf{e}_1 \times
  \mathbf{B}) \cdot \left( \frac{\partial f_p}{\partial v_{\perp}}
  \mathbf{e}_1 + \frac{\partial f_p}{\partial v_{\parallel}} \mathbf{e}_z +
  \frac{1}{v_{\perp}} \frac{\partial f_p}{\partial \alpha} \mathbf{e}_{\alpha}
  \right) \nonumber\\
  & = & \frac{q}{m} (v_{\perp} \mathbf{e}_1 \times \mathbf{B}) \cdot \left(
  \frac{1}{v_{\perp}} \frac{\partial f_p}{\partial \alpha} \mathbf{e}_{\alpha}
  \right) \nonumber\\
  & = & \frac{B_0 q}{m}  \frac{\partial f_p}{\partial \alpha}  (\mathbf{e}_1
  \times \mathbf{e}_z) \cdot \mathbf{e}_{\alpha} \nonumber\\
  & = & - \Omega \left( \frac{\partial f_p}{\partial \alpha} \right), 
\end{eqnarray}
where $\Omega = B_0 q / m$ is the gyro-frequency. Then the Vlasov equation
(\ref{22-11-12-1}) is written as
\begin{equation}
  \frac{\partial f_p}{\partial t} + v_x \frac{\partial f_p}{\partial x} + v_y
  \frac{\partial f_p}{\partial y} + v_z \frac{\partial f_p}{\partial z} -
  \Omega \left( \frac{\partial f_p}{\partial \alpha} \right) = 0,
\end{equation}
Multiplying the above equation by $2 \pi / \Omega f_p$, we get
\begin{equation}
  \label{22-11-12-2} \frac{1}{f_p} \frac{2 \pi}{\Omega_0} \frac{\partial
  f_p}{\partial t} + \frac{1}{f_p} \frac{2 \pi}{\Omega_0} v_x \frac{\partial
  f_p}{\partial x} + \frac{1}{f_p} \frac{2 \pi}{\Omega_0} v_y \frac{\partial
  f_p}{\partial y} + \frac{1}{f_p} \frac{2 \pi}{\Omega_0} v_z \frac{\partial
  f_p}{\partial z} - \frac{2 \pi}{f_p} \left( \frac{\partial f_p}{\partial
  \alpha} \right) = 0.
\end{equation}
...
\begin{equation}
  \frac{1}{f_p}  \frac{2 \pi}{\Omega} v_z \frac{\partial f_p}{\partial z} \sim
  O (\varepsilon^1),
\end{equation}
where $2 \pi v_z / \Omega$ is the parallel distance traveled by a particle
within one gyro-period.

Dropping terms of $O (\varepsilon^1)$, Eq. (\ref{22-11-12-2}) is written as
\begin{equation}
  \frac{1}{f_p} \frac{2 \pi}{\Omega_0} v_x \frac{\partial f_p}{\partial x} +
  \frac{1}{f_p} \frac{2 \pi}{\Omega_0} v_y \frac{\partial f_p}{\partial y} -
  \frac{2 \pi}{f_p} \left( \frac{\partial f_p}{\partial \alpha} \right) = 0,
\end{equation}
i.e.,
\begin{equation}
...
v_{\perp}', \alpha')$, the distribution function $f_p$ is independent of
$\alpha'$. Then working in the new coordinate system is beneficial for
numerically solving the Vlasov equation because the dimension of the problem
is reduced by one.
...
The gyro-angle symmetry for the above simple case is found by using the
...
field plus an electromagnetic perturbation of small amplitude (low-frequency,
$k_{\parallel} \rho_i \ll 1$, and $k_{\perp} \rho_i \sim 1$.).
...
\appendix\section{Distribution function transform**check}
...
\section{Implementation of gyrokinetics in Monte-Carlo simulations}
...
can calculate the guiding-center position $\mathbf{X}$ by using Eq.
(\ref{16-9-21-1}). This is done for only once at the initialization stage of
the simulation. Most codes directly set guiding-center location $\mathbf{X}$,
then the transformation is not needed, but I prefer the former method because
particle coordinates are physical and guiding-center coordinates $\mathbf{X}$
are only imaginary and auxiliary. Update: However, directly sampling particle
position, rather than guiding-center position, is not practical because we
need to load a distribution function that is independent of $\alpha$ in
$(\mathbf{X}, v_{\perp}, v_{\parallel}, \alpha)$ coordinates and this is hard
to achieve if we directly sample $(\mathbf{x}, v_{\perp}, v_{\parallel},
\alpha)$.
...
$\alpha$. The evolution can be computed by using the local gyro-frequency if
...
Monte-Carlo integration. When doing the deposition, the corresponding particle
location is calculated by using an arbitrary gyro-angle, which is of course
...
Monte-Carlo integral because the phase error will cancel each other when
summing over particles. (Wrong! no reason to believe that the error will
cancel each other.) Statistically, we still obtain consistent estimation of
the integral. To further reduce the possible error, we select several (rather
than just one) possible gyro-angles for each marker and average all these over
all markers, yielding more accurate estimation.
...
\end{equation}
where $\delta t$ is a typical gyro-period. In this small time interval,
...
**to be clean up***The answer lies in the important fact that the distribution
...
determined before we can transform a particle location to a guiding-center
...
From the view of particle simulations, the gyrokinetic model can be considered
as a noise reduction method, where the averaging over the gyro-angle is
equivalent to a time averaging over a gyro-period, which reduces the
fluctuation level (in both time and space) associated with evaluating the
