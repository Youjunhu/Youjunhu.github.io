...
\usepackage{geometry,amsmath,graphicx,hyperref,xcolor}
\geometry{a4paper}
...
\tmcolor{blue}{Note that $\nabla \equiv \partial / \partial \mathbf{X}$, which
is taken in the guiding-center coordinates $(\mathbf{X}, \varepsilon, \mu,
\alpha)$ while holding $(\varepsilon, \mu, \alpha)$ constant. How do we
numerically calculate $\partial \Phi_g / \partial \mathbf{X}$ in PIC
simulations? The difficulty is that $\mathbf{X}$ grid is not available, which
makes it difficult to use the finite difference method. We note $\mathbf{x}$
grid is available in PIC simulations. It turns out we can make use of
$\mathbf{x}$ grid to approximatly calculate the derivatives in $\mathbf{X}$
space. Using $\mathbf{x}=\mathbf{X}+\mathbf{\rho}$ and the definition $\delta
\Phi_g (\mathbf{X}, \alpha, v_{\perp}) = \delta \Phi_p (\mathbf{x})$ we obtain
\begin{eqnarray}
  \frac{\partial \delta \Phi_g (\mathbf{X}, \alpha, v_{\perp})}{\partial
  \mathbf{X}} & = & \frac{\partial \delta \Phi_p (\mathbf{x})}{\partial
  \mathbf{X}} \nonumber\\
  & = & \frac{\partial \delta \Phi_p (\mathbf{x})}{\partial \mathbf{x}} \cdot
  \frac{\partial \mathbf{x}}{\partial \mathbf{X}} \nonumber\\
  & = & \frac{\partial \delta \Phi_p (\mathbf{x})}{\partial \mathbf{x}} \cdot
  \left( 1 + \frac{\partial \mathbf{\rho}}{\partial \mathbf{X}} \right)
  \nonumber\\
  & \approx & \frac{\partial \delta \Phi_p (\mathbf{x})}{\partial
  \mathbf{x}}, 
\end{eqnarray}
which is accurate up to $O (\lambda)$ in gyrokinetic ordering. The above
relation indicates that the value of $\partial \delta \Phi_g / \partial
\mathbf{X}$ at a guidng-center location is approximately equal to the value of
$\partial \delta \Phi_g / \partial \mathbf{x}$ at the corresponding particle
position. Therefore we can use $\delta \Phi$ defined on $\mathbf{x}$ grid to
compute \ $\partial \delta \Phi_p / \partial \mathbf{x}$, and interpolate it
to the particle position and this is a good approximation of $\partial \delta
\Phi_g / \partial \mathbf{X}$.

Similarly, we have $\partial \delta A_{\parallel g} / \partial \mathbf{X}
\approx \partial \delta A_{\parallel p} / \partial \mathbf{x}$.

Also note that $\langle \delta \Phi_g \rangle$ is not known on $\mathbf{X}$
grid (it is known on random markers). So, to calculate $\partial \langle
\delta \Phi_g \rangle / \partial \mathbf{X}$, we need to exchange the
operation order:
\begin{equation}
  \frac{\delta \langle \delta \Phi_g \rangle}{\partial \mathbf{X}} = \langle
  \frac{\partial \delta \Phi_g}{\partial \mathbf{X}} \rangle \approx \langle
  \frac{\partial \delta \Phi_p}{\partial \mathbf{x}} \rangle .
\end{equation}
For notation ease, the subscripts $g$ and $p$ are usually dropped. Which one
is intended should be obvious from the context.}
...
\begin{figure}[h]
  \resizebox{238pt}{237pt}{\includegraphics{../figures/double_gyro/p-1.pdf}}
...
\subsection{Poloidal grid}

\begin{figure}[h]
  \includegraphics{/home/yj/theory/figures/my_grids_and_gem_grids/poloidal_grids/a-1.pdf}
  \caption{Poloidal grid for equilibrium quantities used in \tmtexttt{TEK} and
  \tmtexttt{GEM} code. The array starts at $\theta = - \pi$ and ends at
  $\theta = + \pi$ ($\theta = \pm \pi$ is chosen to be at the high-field side
  midplane in both the codes). \tmtexttt{TEK} array index starts from 1
  whereas GEM array index starts from $0$. Hence \tmtexttt{mpol=nth+1}.
  \tmtexttt{nth} is denoted by \tmtexttt{ntheta} in GEM. In TEK,
  \tmtexttt{mpol} must be an odd number, so that the array has a midpoint,
  whose index is given by \tmtexttt{(mpol+1)/2}, which corresponds to $\theta
  = 0$.}
\end{figure}

I do not need to make connection with GEM's equilibrium poloidal array because
there is no coupling of equilibrium quantities between the code written by me
and the original code in GEM. The coupling happens for the perturbed quanties,
whose poloidal grids need to be consistent.


\begin{itemize}
  \item The gridpoint number along $\theta$ for perturbations is denoted by
  \tmtexttt{mpol2}. Its value is not chosen by users. Instead, it is equal to
  \tmtexttt{numproc/ntube}, i.e., \tmtexttt{mpol2} is the total number of
  cells along $\theta$ for MPI parallelation. (\tmtexttt{numproc} and
  \tmtexttt{ntube} must be chosen in a way that makes \
  \tmtexttt{numproc/ntube} an integer.)
  
  \item Poloidal grid-points for perturbations are indexed as
  \tmtexttt{0:mpol2-1}, with \tmtexttt{0} corresponding to $\theta = - \pi$
  and \tmtexttt{mpol2} corresponding to $\theta = \pi$. Field equations are
  solved at \tmtexttt{$0 : \tmop{mpol} 2 - 1$}. (The field at
  \tmtexttt{mpol2}, i.e., $\theta = + \pi$, is not identical to that at
  $\theta = - \pi$ due to toroidal shift of a magnetic field line when it
  finish one poloidal loop, and is obtained by toroidally inerpolating the
  fields at $\theta = - \pi$.) No array for this is actually used in the code.
  The indexes just correspond to \tmtexttt{gclr} of MPI processors.
  
  \item The equilibrium poloidal gridpoint number, \tmtexttt{mpol}, must be
  chosen in a way that makes \tmtexttt{(mpol-1)/mpol2} an integer, which can
  be larger than 1, i.e., one simulation cell can span several equilibrium
  cells.
\end{itemize}
...
\begin{figure}[h]
  ...
  \caption{Toroidal grid used in \tmtexttt{TEK} and GEM. In TEK, the toroidal
  array is \tmtexttt{tor\_1d\_array(1:mtor+1)}.  Source terms, $\delta n$ and $\delta j_{\parallel}$, and EM fields are
  defined at \tmtexttt{1:mtor}.
  GEM array index starts from $0$ and ends at \tmtexttt{jm}. It follows that
  \tmtexttt{mtor=jm}.}
\end{figure}
...
\begin{figure}[h]
  \resizebox{0.9\columnwidth}{!}{\includegraphics{/home/yj/theory/figures/my_grids_and_gem_grids/radial_grids/a-1.eps}}
  \caption{Radial grid used in \tmtexttt{TEK}. Two radial arrays are used in
  the code, \tmtexttt{radcor\_1d\_array(1:nt)} and
  \tmtexttt{radcor\_1d\_array2(1:n)}, equilibrium is defined on the former
  grid, and perturbations are defined on the latter grid. Here $\tmop{nt} = n
  + 2 m$, $m$ is the number of grid points in one of the two buffer regions
  (regions in blue color). In the code $n$ is denoted by \tmtexttt{nflux2} and
  $\tmop{nt}$ is denoted by \tmtexttt{nflux}, $m$ is denoted by
  \tmtexttt{points\_in\_buffer}. In \tmtexttt{GEM} the radial array does no
  include the buffer regions and the index starts at \tmtexttt{0} and ends at
  \tmtexttt{imx}. Hence \tmtexttt{n=imx+1}.}
\end{figure}

GEM array index system is better than TEK's because my system is not
consistent: sometimes I use 0-based index and sometimes I use 1-based index,
sometimes the index ends at n and sometimes ends at n+1. It is important to
know accurately the transformation between the two systems.
...
TeX format and then using \tmtexttt{htlatex} to convert the TeX file to HTML
format.
...
\begin{thebibliography}{1}
  ...
  \bibitem[4]{MISHCHENKO2019194}A.~Mishchenko, A.~Bottino, A.~Biancalani,
  R.~Hatzky, T.~Hayward-Schneider, N.~Ohana, E.~Lanti, S.~Brunner, L.~Villard,
  M.~Borchardt, R.~Kleiber, and  A.~K{\"o}nies. {\newblock}Pullback scheme
  implementation in orb5. {\newblock}\tmtextit{Computer Physics
  Communications}, 238:194--202, 2019.{\newblock}
  
  \bibitem[5]{texmacs}Joris van der~Hoeven. {\newblock}Gnu texmacs, a
