...
\tmcolor{blue}{Using Eq. (\ref{2-25-p7}) in Eqs. (\ref{2-25-p6}), we obtain
\begin{eqnarray}
...
(\ref{23-4-3-4}) and line (\ref{23-4-3-5}), the above expression is reduced to
\begin{eqnarray}
  h (x) & = & \sum_{n = - \infty}^{\infty} \left( \frac{1}{2 L} \int_{- L}^L h
...
Denote the summation in Eq. (\ref{10-3-e2m}) by $H_n$:
\begin{equation}
  \label{2-26-2} H_n \equiv \sum_{j = 0}^{N - 1} h_j \exp \left( - i \frac{2
  \pi}{N} n j \right) .
\end{equation}
Then $H_n$ with $n = 0, 1, \ldots N - 1$ is called the {\tmstrong{Discrete
Fourier transformation (DFT)}} of $h_j$ with $j = 0, 1, \ldots, N - 1$. (The
efficient algorithm of computing the DFT is discussed in Appendix
\ref{18-1-28-a1}.)

Why do we choose $n$ to be in the positive range $[0 : N - 1]$? This is to
make the tranform pair symmetric: both $j$ and $n$ are in the same range. This
symmetry is helpful in making the transform pair easy to remember. This does
not mean we only need the positive frequency components. In fact, we usually
need $n$ in the range $[- N / 2, N / 2]$. This will be discussed in the next
section.

\tmcolor{blue}{In terms of $H_n$, the Fourier expansion coefficients $c_n$ in
Eq. (\ref{10-3-e2m}) is written as
\begin{equation}
  c_n = \frac{1}{N} H_n .
\end{equation}
The corresponding frequency is
\begin{equation}
  \label{10-26-1} f_n = \frac{n}{T} = \frac{n}{\Delta N} = f_s \frac{n}{N} .
\end{equation}
Here $f_1$ is called the fundamental frequency (spacing in the frequency
domain).}

\

\subsection{Periodic property of DFT}
